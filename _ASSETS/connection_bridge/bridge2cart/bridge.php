  ...
  public function __construct()
  {
    ...
    if (!$check || file_exists($baseDir . 'app/etc/env.php')) {
      ...
      if (@file_exists(M1_STORE_BASE_DIR . 'composer.json')) {
        $ver = file_get_contents(M1_STORE_BASE_DIR . 'composer.json');
  ...
  public function magentoClearCache()
  {
    ...
    if ($phpExecutable) {
      ...
      if (file_exists(M1_STORE_BASE_DIR . 'app/etc/env.php')) {
        $indexer = "`$phpExecutable $memoryLimit bin/magento indexer:reindex`";
        $imagesResize ="`$phpExecutable $memoryLimit bin/magento catalog:images:resize`";
        $clearCache = "`$phpExecutable $memoryLimit bin/magento cache:flush`";
        @exec("$indexer &");
        @exec("$imagesResize &");
        @exec("$clearCache &");
        ...
        @exec($phpExecutable . " shell/indexer.php --reindexall");
        //$execStatus = $this->checkMagentoOutPut($outPut);
      }
    } else {
